...
class XcfaToMonolithicAdapter(private val xcfa: XCFA, private val initValues: Boolean = false) :
  ...
  protected lateinit var locVar: VarDecl<Type>
  protected lateinit var locations: List<XcfaLocation>
  ...
  override fun modelToMonolithicExpr(parseContext: ParseContext): MonolithicExpr {
    ...
    val tranList =
      proc.edges
        .map { edge: XcfaEdge ->
          ...
          SequenceStmt.of(
            listOf(
              AssumeStmt.of(Eq(locVar.ref, int(locationMap[source]!!))),
              label.toStmt(),
              AssignStmt.of(locVar, cast(int(locationMap[target]!!), locVar.type)),
    ...
    return MonolithicExpr(
      initExpr =
        And(
          Eq(locVar.ref, int(locationMap[proc.initLoc]!!)),
          Eq(edgeVar.ref, int(-1)),
          defaultValues,
        ),
      transExpr = And(transUnfold.exprs),
      propExpr =
        if (proc.errorLoc.isPresent) Neq(locVar.ref, int(locationMap[proc.errorLoc.get()]!!))
        else True(),
          ...
          Pair(
            ...
            listOf(
              PredState.of(
                ExprUtils.simplify(
                  proof.getInvariant(),
                  ImmutableValuation.builder().put(locVar, Int(index)).build(),
                )
              )
            ),
          )
