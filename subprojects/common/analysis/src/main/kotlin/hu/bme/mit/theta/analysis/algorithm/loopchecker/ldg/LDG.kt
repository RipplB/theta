...
class LDG<S : ExprState, A : ExprAction>(
    ...
    val nodes = HashMap<S, LDGNode<S, A>>()
    ...
    fun getOrCreateNode(state: S): LDGNode<S, A> = nodes.computeIfAbsent(state) { s ->
        LDGNode(
            s, acceptancePredicate.test(Pair(s, null))
        )
    }
