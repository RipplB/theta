...
class MilanoLDGTraceCheckerStrategy<S: ExprState, A : ExprAction>(private val trace: LDGTrace<S, A>, solverFactory: SolverFactory, init : Expr<BoolType>, logger: Logger) : AbstractLDGTraceCheckerStrategy<S, A>(trace, solverFactory, init, logger) {
    ...
    override fun evaluateLoop(valuation: Valuation): ExprTraceStatus<ItpRefutation> {
        for (variable in variables) {
            ...
            val interpolantExpr: Expr<BoolType> = ExprUtils.simplify(foldIn(interpolant.eval(satMarker), indexings[stateCount - 1], valuation))
    ...
    private fun <T : Type> foldIn(expr: Expr<T>, indexing: VarIndexing, valuation: Valuation): Expr<T> {
