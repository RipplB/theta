...
class BoundedUnrollingLDGTraceCheckerStrategy<S : ExprState, A : ExprAction>(
    private val trace: LDGTrace<S, A>, private val solverFactory: SolverFactory,
    init: Expr<BoolType>, private val bound: Int, private val logger: Logger
    ...
    private fun findSmallestAbstractState(i: Int, bound: Int, usedVariablesPrecision: ExplPrec): Int {
        ...
        val statesForExpr: Collection<ExplState> = ExprStates.createStatesForExpr(
            solverFactory.createSolver(), expr, 0, usedVariablesPrecision::createState, VarIndexingFactory.indexing(0),
            bound
        )
        val currentSize: DomainSize = statesForExpr
            .map {
                usedVariablesPrecision
                    .vars
                    .filterNot(ExprUtils.getVars(it.toExpr())::contains)
                    .map(VarDecl<*>::getType)
                    .map(Type::getDomainSize)
                    .fold(DomainSize.ONE, DomainSize::multiply)
            }
            .fold(DomainSize.ZERO, DomainSize::add)
        return if (currentSize.isInfinite || currentSize.isBiggerThan(bound.toLong())) findSmallestAbstractState(
            i + 1, bound, usedVariablesPrecision
        )
    ...
    fun expandUsedVariables(usedVariables: Set<VarDecl<*>>): Set<VarDecl<*>> {
        val expanded = trace.loop.fold(emptySet<VarDecl<*>>()) { acc, edge ->
            if (edge.action is StmtAction) VarCollectorStmtVisitor.visitAll(
                edge.action.getStmts(), acc
            ) else emptySet()
        }

        return if (expanded.size > usedVariables.size) expandUsedVariables(expanded)
        else usedVariables
    }
    ...
    private fun putLoopOnSolver(marker: ItpMarker, startIndexing: VarIndexing) {
        ...
        solver.add(
            marker, PathUtils.unfold(
                trace.loop[trace.loop.size - 1].target.state.toExpr(), loopIndexing
            )
        )
    }
}