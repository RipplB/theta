...
class LdgVisualizer<S : ExprState, A : ExprAction> (
    ...
    private fun createVisualization(ldg: LDG<out S, out A>): Graph {
        ...
        for (initNode in ldg.initNodes) {
            ...
            val nAttributes = NodeAttributes.builder().label("")
            ...
            val eAttributes = EdgeAttributes.builder().label("")
                .color(if (traceNodes.contains(initNode)) TARGET_COLOR else LINE_COLOR)
                .lineStyle(SUCC_EDGE_STYLE).build()
            graph.addEdge(
                PHANTOM_INIT_ID + initNode.id, NODE_ID_PREFIX + initNode.id,
                eAttributes
            )
        }
    ...
    private fun traverse(
        graph: Graph, node: LDGNode<out S, out A>,
        ...
        val nAttributes = NodeAttributes.builder()
