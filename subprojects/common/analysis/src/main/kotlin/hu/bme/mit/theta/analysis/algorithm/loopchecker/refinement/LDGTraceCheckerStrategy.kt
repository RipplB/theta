...
enum class LDGTraceCheckerStrategy {
    MILANO {
        ...
        override fun <S : ExprState, A : ExprAction> check(
            ...
            try {
                return BoundedUnrollingLDGTraceCheckerStrategy(trace, solverFactory, init, 100, logger).check()
...
abstract class AbstractLDGTraceCheckerStrategy<S: ExprState, A : ExprAction>(private val trace: LDGTrace<S, A>, solverFactory: SolverFactory, private val init : Expr<BoolType>, private val logger: Logger) {
    ...
    private fun infeasibleAsLoopIsUnreachable(satPrefix: Int): ExprTraceStatus<ItpRefutation> {
        ...
        solver.add(
            unreachableMarker, PathUtils.unfold(
                trace.getState(satPrefix + 1).toExpr(),
                indexings[satPrefix + 1]
            )
        )
        solver.add(
            unreachableMarker, PathUtils.unfold(
                trace.getAction(satPrefix)!!.toExpr(),
                indexings[satPrefix]
            )
        )
    ...
    protected fun infeasibleThroughInterpolant(satPrefix: Int, indexing: VarIndexing): ExprTraceStatus<ItpRefutation> {
        solver.check()
