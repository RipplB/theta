...
class LDGTrace<S : ExprState, A : ExprAction>(
    ...
    constructor(edges: List<LDGEdge<S, A>>, honda: LDGNode<S, A>) : this(
        edges.takeWhile { it.source != honda }, honda, edges.dropWhile { it.source != honda })

    init {
        check(
            (1 until tail.size).none { tail[it - 1].target != tail[it].source }) { "The edges of the tail have to connect into each other" }
        check(tail.last().target == honda) { "The tail has to finish in the honda" }
        check(loop.first().source == honda) { "The loop has to start in the honda" }
        check(
            (1 until loop.size).none { loop[it - 1].target != loop[it].source }) { "The edges of the loop have to connect into each other" }
    ...
    fun toTrace(): Trace<S, A> = Trace.of(edges.map { it.source!!.state } + honda.state, edges.map { it.action!! })

    fun print(logger: Logger, level: Level) {
        tail.forEach { logger.write(level, "%s%n---through action:---%n%s%n--------->%n", it.source?.state, it.action) }
        logger.write(level, "---HONDA:---%n{ %s }---------%n", honda.state)
        loop.forEach { logger.write(level, "---through action:---%n%s%n--------->%n%s%n", it.action, it.target.state) }
    }
}