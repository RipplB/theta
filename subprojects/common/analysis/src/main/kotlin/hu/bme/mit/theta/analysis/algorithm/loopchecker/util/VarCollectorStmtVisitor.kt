...
class VarCollectorStmtVisitor : StmtVisitor<Set<VarDecl<*>>, Set<VarDecl<*>>> {
    ...
    companion object {
        ...
        fun visitAll(stmts: Collection<Stmt>, vars: Set<VarDecl<*>>): Set<VarDecl<*>> {
            ...
            val expanded: Set<VarDecl<*>> = stmts.fold(emptySet()) { acc, stmt -> acc + stmt.accept(visitor, vars) }
    ...
    override fun <DeclType : Type> visit(stmt: HavocStmt<DeclType>, param: Set<VarDecl<*>>) = param + stmt.varDecl

    override fun visit(stmt: SequenceStmt, param: Set<VarDecl<*>>) = param + visitAll(stmt.stmts, param)
