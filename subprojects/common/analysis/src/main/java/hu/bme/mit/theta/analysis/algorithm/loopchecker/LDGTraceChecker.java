...
public final class LDGTraceChecker<S extends ExprState, A extends ExprAction> {
	...
	private int findSmallestAbstractState(int i, int bound, ExplPrec usedVariablesPrecision) {
		...
		Collection<? extends ExplState> statesForExpr = ExprStates.createStatesForExpr(Z3LegacySolverFactory.getInstance().createSolver(), expr, 0, func, VarIndexingFactory.indexing(0), bound);
