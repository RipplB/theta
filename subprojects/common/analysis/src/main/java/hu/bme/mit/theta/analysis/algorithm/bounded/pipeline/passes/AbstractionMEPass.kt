...
import hu.bme.mit.theta.analysis.algorithm.bounded.pipeline.*
...
/**
 * Abstraction pass for monolithic expressions. Currently only supports predicate abstraction by
 * substituting predicates into the expressions.
 */
class AbstractionMEPass<P : PredPrec, Pr : Proof>(
  val initPrec: P = PredPrec.of() as P,
  private val traceCheckerFactory: (MonolithicExpr) -> ExprTraceChecker<ItpRefutation>,
  val refine: (P, Expr<BoolType>) -> P = { prec, expr -> prec.join(PredPrec.of(expr)) as P },
