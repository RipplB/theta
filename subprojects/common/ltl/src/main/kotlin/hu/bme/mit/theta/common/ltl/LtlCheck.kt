...
object LtlCheck {
    fun <RState : ExprState, RBlank : ExprState, RAction : ExprAction, RPrec : Prec, RBlankPrec : Prec, DataPrec : Prec, DataState : ExprState> check(
        ...
        solver: ItpSolver,
        logger: Logger
    ): Optional<Trace<ExprMultiState<RBlank, CfaState<UnitState>, DataState>, ExprMultiAction<RAction, CfaAction>>> {
        ...
        return verifier.verify(MultiPrec(initPrec, GlobalCfaPrec.create(dataInitPrec), dataInitPrec), SearchStrategy.DFS, RefinerStrategy.BOUNDED_UNROLLING)
    }
    ...
    private fun defineNextSide(state: MultiState<*, *, *>) : MultiSourceSide {
