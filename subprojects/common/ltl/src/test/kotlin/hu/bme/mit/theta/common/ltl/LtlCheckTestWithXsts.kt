...
class LtlCheckTestWithXsts(
    ...
    private val logger: Logger = ConsoleLogger(Logger.Level.MAINSTEP)
...
//            arrayOf("counter6to7", "G F(x=7)", true),
    ...
    fun test() {
        ...
        val configBuilder = XstsConfigBuilder(
            XstsConfigBuilder.Domain.EXPL, XstsConfigBuilder.Refinement.SEQ_ITP, solverFactory, solverFactory
        ).initPrec(XstsConfigBuilder.InitPrec.EMPTY).ExplStrategy(xsts)
        ...
        val checker = LtlChecker(
            ...
            xsts.initFormula,
            SingleXstsNextSideFunction({ it.leftState }, MultiSide.LEFT)
        )
