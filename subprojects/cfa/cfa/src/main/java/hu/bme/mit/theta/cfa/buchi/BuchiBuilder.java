...
public final class BuchiBuilder implements HOAConsumer {
    ...
    private Expr<BoolType> apBoolExpressionToInternal(BooleanExpression<AtomLabel> booleanExpression) {
        return switch (booleanExpression.getType()) {
            case EXP_AND ->
                    BoolExprs.And(apBoolExpressionToInternal(booleanExpression.getLeft()), apBoolExpressionToInternal(booleanExpression.getRight()));
            case EXP_OR ->
                    BoolExprs.Or(apBoolExpressionToInternal(booleanExpression.getLeft()), apBoolExpressionToInternal(booleanExpression.getRight()));
            case EXP_NOT -> BoolExprs.Not(apBoolExpressionToInternal(booleanExpression.getLeft()));
            case EXP_TRUE -> BoolExprs.True();
            case EXP_ATOM ->
                    swappedExpressions.get(aps.get(Integer.parseInt(booleanExpression.getAtom().toString())));
